...
class MiddlewaresConsumer {
  ...
  static init() {
    ...
    this.storges = {
      ...
      controller: (controller, routePath, isRestful) =>
           ([ ...new Set(this.scanForPaths(controller, routePath, isRestful) ).values()]),
  ...
  // routes = ['/xxx', '/yyy', [path:'zjl'] ,controller.XXX ]
  ...
  static resolveRouter(path) {
    const verb = this.getMethod();
    verb(path, ...this.middlewares);
  }
  ...
  static getMethod() {
    return this.router.all.bind(this.router);
  }
