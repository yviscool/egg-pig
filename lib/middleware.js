...
class MiddlewaresConsumer {
  ...
  static getMappingController(route) {
    // loadController 阶段会往 controller.prototype 设置一个 pathName;  controller.foo.bar
    const fullPath = route[FULLPATH];
    const baseName = path.basename(fullPath); //  foo.ts/js
    const key = baseName.substring(0, baseName.indexOf('.')); // foo
    const pathProperties = this.modules.get(key);
