...
class MiddlewaresConsumer {
  ...
  static init() {
    ...
    this.storges = {
      ...
      path: ({ path, method }) => [ this.validateRouterPath(path), method ],

      controller: controllerInfo => this.scanForPaths(controllerInfo),

    };

    this.routers = require('./consumer').getRouters();
  ...
  static mapRouteToRouteProps(route) {
    ...
    if (route.path && route.method) return this.storges.path(route);
  ...
  static resolveRouter([ path, method ]) {
    ...
    routerMethod(path, ...this.middlewares);
  }
  ...
  static scanForPaths({ controller, prefix }) {
    ...
    return Object
      ...
      .filter(method => this.reflectMethodPath(proto, method) || Object.keys(REST_MAP).find(key => proto[key]))
