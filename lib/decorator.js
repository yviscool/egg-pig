...
const DecoratorManger = require('./decorator_manager');
...
function createParamMapping(paramType, factory) {
  return (data, ...pipes) => {
    return (target, key, index) => {
      ...
      //   },
      // }, metatype, key);
      const args = DecoratorManger.getMethodDataFromClass(ROUTE_ARGS_METADATA, target, key);
      DecoratorManger.saveMethodDataToClass(ROUTE_ARGS_METADATA, {
        ...args,
        [`${paramType}:${index}`]: {
          index,
          factory,
...
function createRouterMapping(methodType = RequestMethod.GET) {
  return (name, paramOptions) => {
    return (target, key, descriptor) => {
      ...
      DecoratorManger.attachClassMetadata(ROUTE_OPTIONS_METADATA, options, metatype);
...
function createUseMapping(metadata) {
  return (...arr) => (target, key, descriptor) => {
    if (descriptor) {
      ...
      DecoratorManger.saveMethodDataToClass(metadata, arr, target, key);
      return descriptor;
    }
    validEachFactory(metadata, arr, target);
    // Reflector.set(metadata, arr, target);
    DecoratorManger.saveMethodDataToClass(metadata, arr, target);
    return target;
  };
}
...
exports.Catch = function Catch(...exceptions) {
  return target => {
    ...
    DecoratorManger.attachClassMetadata(FILTER_CATCH_EXCEPTIONS, exceptions, target);
  };
};
...
exports.Controller = function Controller(prefix = '/', routerOptions = { middleware: [], sensitive: true }) {
  return target => {
    ...
    //   routerOptions,
    // }, target);
    DecoratorManger.saveModule(CONTROLLER_MODULE_METADATA, target);
    DecoratorManger.saveClassMetadata(PATH_METADATA, {
      name: '',
      prefix: validatePath(prefix),
      isRestful: false,
      routerOptions,
...
exports.Priority = function Priority(priority = 0) {
  return target => {
    ...
    DecoratorManger.saveClassMetadata(PRIORITY_METADATA, priority, target);
  };
};
...
exports.HttpCode = function HttpCode(statusCode) {
  return (target, key) => {
    ...
    DecoratorManger.saveMethodDataToClass(HTTP_CODE_METADATA, statusCode, target, key);
  };
};
...
exports.Render = function Render(template) {
  return (target, key) => {
    ...
    DecoratorManger.saveMethodDataToClass(RENDER_METADATA, template, target, key);
  };
};
...
exports.Header = function Header(name, value) {
  return (target, key) => {
    ...
    DecoratorManger.attachMethodDataToClass(HEADER_METADATA, { name, value }, target, key);
  };
};
