...
const validGuard = guard => guard && is.object(guard) && is.function(guard.canActivate);
const validPipe = pipe => pipe && is.object(pipe) && is.function(pipe.transform);
const validInterceptor = interceptor => interceptor && is.object(interceptor) && is.function(interceptor.intercept);
const validFilter = filter => filter && is.object(filter) && is.function(filter.catch);
