...
module.exports = {
  ...
  create(proto, key, metadata, method) {
    ...
    return [
      ...this.getsFeaturesInstance(classMetadata, method),
      ...this.getsFeaturesInstance(methodMetadata, method),
    ];
  },

  // get pipes/guards/interceptors/filter instance
  getsFeaturesInstance(metadata, methoeName) {
  ...
  scanForController(controller) {
    const items = this.routerProperties = [];
    const proto = controller.prototype;
    for (const method of Object.getOwnPropertyNames(proto)) {
      if (!is.function(proto[method]) || method === 'constructor') {
        continue;
      }
      items.push({
        method,
