...
module.exports = {
  ...
  resolveRouters() {
    ...
    for (const { routerPaths, routerMetadata, properties } of this.container.values()) {
      ...
      for (const pathProperty of routerPaths) {
        ...
        const routerArgs = paramOptions
          ? [paramOptions.routerName, path, targetcallback].filter(x => x)
          : [path, targetcallback].filter(x => x);
  ...
  createEggRouter(basePath, routerOptions) {
    if (basePath) {
      ...
      router.prefix(this.globalPrefix + basePath);
      return router;
    }
    ...
    if (middlewares && middlewares.length) {
      for (const middleware of middlewares) {
        if (typeof middleware === 'function') {
          ...
          const newMiddleware = this.app.middleware[middleware];
          if (newMiddleware) {
            handlerCallback(newMiddleware);
          }
        }
      }
    }
