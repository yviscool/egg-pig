...
module.exports = {
  ...
  createMethodsProxy() {
    ...
    for (const controller of controllerModules) {
      const controllerMetadta = DecoratorManager.getClassMetadata(PATH_METADATA, controller);
      const priorityMetadata = DecoratorManager.getClassMetadata(PRIORITY_METADATA, controller);
      const routerMetadatas = DecoratorManager.getMethodDataFromClass(ROUTE_OPTIONS_METADATA, controller);
      for (const routerMetadata of routerMetadatas) {
        const targetCallback = routerProxy.createCallbackProxy({ method: routerMetadata.mehtod, targetCallback: routerMetadata.targetCallback }, controller);
        Object.assign(routerMetadatas, { targetCallback });
      }
      this.resolveRouters(controllerMetadta, priorityMetadata, routerMetadatas);
    }
  ...
  resolveRouters(controllerMetadta, priority, routerMetadatas) {
    ...
    for (const pathProperty of routerMetadatas) {
      ...
      const { path, paramOptions, requestMethod, targetCallback } = pathProperty;
