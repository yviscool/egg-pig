    ...
    for (const controller of controllerModules) {
      ...
      if (Array.isArray(routerMetadatas) && routerMetadatas.length !== 0 ){
        for (const routerMetadata of routerMetadatas) {
          const targetCallback = routerProxy.createCallbackProxy({ method: routerMetadata.method, targetCallback: routerMetadata.targetCallback }, controller);
        ...
        if (controller.isRestful){
          for (const key of REST_MAP){
            ...
            if (targetCallback){
              const proxyTargetCallback = routerProxy.createCallbackProxy({ method: key, targetCallback }, controller);
              Object.assign(routerMetadatas, { targetCallback: proxyTargetCallback });
            }
          }
        }
        this.resolveRouters(controllerMetadata, priorityMetadata, routerMetadatas);
      } 
    }
