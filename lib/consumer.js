...
module.exports = {
    ...
    reflectMethodMetadata(proto, method) {
        ...
        const controlerMetadata = Reflect.getMetadata(PATH_METADATA, proto.constructor);
        const requestMethod = Reflect.getMetadata(METHOD_METADATA, proto, method);
        const routeName = Reflect.getMetadata(ROUTE_NAME_METADATA, proto, method);
        name = [name, controlerMetadata.prefix].join(':')
        const module = this.modules[name] ? this.modules[name] : this.modules[name] = [];
        module.push({
            ...controlerMetadata, // { name, prefix, isRestful}
            path,
            proto,
            method,
            routeName,
    ...
    resolveRouters(app) {
        Object.keys(this.modules).map(key => {
            ...
            for (const pathProperty of pathProperties) {
                const {
                    name, //resources route name
                    prefix, // controller prefix
                    isRestful, // true false
                    path,
                    proto,
                    method,
                    routeName,
                ...
                if (!isRestful) {
                    ...
                    const args = routeName ? [routerName, path, targetcallback] : [path, targetcallback];
                    routerMethod(...args);
                } else {
