...
module.exports = {
    ...
    createCallbackProxy({ proto, method, targetCallback }) {
        ...
        this.reflectMethodMetadata(proto, method);
    ...
    reflectMethodMetadata(proto, method) {
        const path = Reflect.getMetadata(PATH_METADATA, proto, method);
        const prefix = Reflect.getMetadata(PATH_METADATA, proto.constructor);
        const requestMethod = Reflect.getMetadata(METHOD_METADATA, proto, method);
        const module = this.modules[prefix] ? this.modules[prefix] : this.modules[prefix] = [];
        module.push({
            path,
            proto,
            requestMethod,
    ...
    resolveRouters(app) {
        Object.keys(this.modules).map(prefix => {
            const pathProperties = this.modules[prefix];
            const router = this.createRouter(app);
            pathProperties.map(({ path, requestMethod, proto, method }) => {
                const routerMethod = router[RequestMethod[requestMethod]].bind(router);
                const targetcallback = Object.getOwnPropertyDescriptor(proto, method).value;
                routerMethod(path, this.methodToMiddleware(targetcallback));
            })
            this.getRouter(app).use(prefix, router.routes())
        })
