...
module.exports = {
    ...
    return function (ctx) {
      switch (key) {
        ...
        case RouteParamTypes.FILE:
          return ctx.getFileStream && ctx.getFileStream();
        case RouteParamTypes.FILES:
          return ctx.multipart && ctx.multipart();
        default:
          return null;
      }
    };
  ...
  createPipesFn(pipes, callbackParamtypes, parmTypes, getPipeFn) {
    ...
    return async function (args) {
      await Promise.all(callbackParamtypes.map(async param => {
        ...
        const value = extractValue(this.ctx);
        ...
        if (
          ...
          const returnValue = await pipes.concat(paramPipes).reduce(async (value, transform) => {
            const val = await value;
            const result = transform.call(this, val, {
              data,
              metaType,
              type: paramType,
            });
  ...
  createCallbackProxy({ proto, method, targetCallback }) {
    ...
    const canTransformFn = this.createPipesFn(pipes, callbackParamtypes, paramTypes, this.getPigs);
    ...
    Object.defineProperty(proto, method, {
      async value() {
        ...
        // intercept
        await interceptorFn.call(this, this.ctx.request, targetCallback, handler);
      },
    }
    );
  },
  ...
  resolveRouters(app) {
    Object.keys(this.modules).forEach(key => {
      ...
      let { name = '', prefix, isRestful, proto } = pathProperties.metadata;
