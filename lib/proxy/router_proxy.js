...
class RouterProxy {
  ...
  createFinalHandler(controller, method) {
    ...
    const httpCode = getClassMetadata(controller, method, HTTP_CODE_METADATA);
    const renderTemplate = getClassMetadata(controller, method, RENDER_METADATA);
    const responseHeaders = getClassMetadata(controller, method, HEADER_METADATA);
    const { requestMethod } = getClassMetadata(controller, method, ROUTE_OPTIONS_METADATA) || {};
  ...
  createCallbackProxy(routerProperty, controller) {
    ...
    const paramTypes = this.reflector.getClassMetadata(proto, method, PARAMTYPES_METADATA);
