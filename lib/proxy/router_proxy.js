...
class RouterProxy {
  ...
  reflectCallbackParamtypes(klass, key) {
    ...
    const args = DecoratorManager.getMethodDataFromClass(ROUTE_ARGS_METADATA, klass, key);
  ...
  createFinalHandler(controller, method) {
    ...
    const httpCode = DecoratorManager.getClassMetadata(HTTP_CODE_METADATA, controller, method);
    const renderTemplate = DecoratorManager.getClassMetadata(RENDER_METADATA, controller, method);
    const responseHeaders = DecoratorManager.getClassMetadata(HEADER_METADATA, controller, method);
    const { requestMethod } = DecoratorManager.getClassMetadata(ROUTE_OPTIONS_METADATA, controller, method);
  ...
  createCallbackProxy(routerProperty, controller) {
    ...
    const paramTypes = Reflect.getMetadata(PARAMTYPES_METADATA, controller, method);
    ...
    Object.defineProperty(controller.prototoype, method, {
      async value() {
    ...
    return controller.prototoype[method];
  }
