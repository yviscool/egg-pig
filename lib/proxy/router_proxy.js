...
class RouterProxy {
  ...
  createFinalHandler(controller, method) {
    ...
    const httpCode = DecoratorManager.getMethodDataFromClass(HTTP_CODE_METADATA, controller, method);
    const renderTemplate = DecoratorManager.getMethodDataFromClass(RENDER_METADATA, controller, method);
    const responseHeaders = DecoratorManager.getMethodDataFromClass(HEADER_METADATA, controller, method);
    const routerOptionsMetadatas = DecoratorManager.getClassMetadata(ROUTE_OPTIONS_METADATA, controller) || [];
  ...
  createCallbackProxy(routerProperty, controller) {
    ...
    const paramTypes = Reflect.getMetadata(PARAMTYPES_METADATA, controller.prototype, method);
