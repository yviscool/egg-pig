...
const { getClassMetadata, getMethodDataFromClass } = require('../decorator_manager');
...
class ContextCreator {
  ...
  create(controller, key, metadata, loadInstanceFnc) {
    ...
    const classMetadata = getClassMetadata(metadata, controller);
    const methodMetadata = getMethodDataFromClass(metadata, controller, key);
    return [
  ...
  loadInstanceForFilters(injectable) {
    ...
    const exceptionMetatypes = (
      isValidFilter
        ? getClassMetadata(FILTER_CATCH_EXCEPTIONS, injectable)
        : getClassMetadata(FILTER_CATCH_EXCEPTIONS, Object.getPrototypeOf(injectable).constructor)
    ) || [];
